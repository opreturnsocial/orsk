...
A pure repost of an existing ORS transaction. Contains only a reference to the original ORS transaction's Bitcoin txid - no added content.

## Format

| Field             | Kind-data offset | Size     | Description                                           |
| ----------------- | ---------------- | -------- | ----------------------------------------------------- |
| `referenced_txid` | 0                | 32 bytes | txid of the ORS transaction being reposted (internal byte order) |
...
Parsers SHOULD verify the referenced ORS transaction exists. Parsers MUST NOT reject a repost solely because the referenced transaction is unknown or not yet seen (e.g. out-of-order scanning).
...
3. `referenced_txid` SHOULD reference a known ORS transaction
