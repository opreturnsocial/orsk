...
A repost of an existing ORS transaction with added commentary. Extends ORK-04 with a UTF-8 content field beginning at kind-data offset 32.

## Format

| Field             | Kind-data offset | Size     | Description                                               |
| ----------------- | ---------------- | -------- | --------------------------------------------------------- |
| `referenced_txid` | 0                | 32 bytes | txid of the ORS transaction being quoted (internal byte order)       |
...
Parsers SHOULD verify the referenced ORS transaction exists. Parsers MUST NOT reject a quote repost solely because the referenced transaction is unknown or not yet seen (e.g. out-of-order scanning).
...
4. `referenced_txid` SHOULD reference a known ORS transaction
